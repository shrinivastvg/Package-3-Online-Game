...
function handleOptionSelect(selectedOption, correctAnswer, element) {
    ...
    if (selectedOption === correctAnswer) {
        ...
        // Correct answer decreases project cost and time
        ...
        // Wrong answer increases cost and time, and tracks excess for final display
        initialCost += costDecrement;
        initialTime += timeDecrement;
        extraCost += costDecrement; // Track the additional cost due to wrong answers
        extraTime += timeDecrement; // Track the additional time due to wrong answers
    }

    // Update the project cost and time displays in the UI
    document.getElementById("project-cost").innerText = `${Math.max(initialCost, 0).toFixed(2)} crore`;
    updateProjectTime(); // Ensure time displays correctly
...
// Ensure project time is displayed properly (Years or Months)
function updateProjectTime() {
    if (initialTime >= 12) {
        document.getElementById("project-time").innerText = "1 Year";
    } else {
        document.getElementById("project-time").innerText = `${Math.max(initialTime, 0).toFixed(1)} Months`;
    }
}
